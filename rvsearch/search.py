...
class Search(object):
    ...
    def __init__(
        ...
        if post == None:
            self.priors = priors
            self.params = utils.initialize_default_pars(
                instnames=self.tels,
                times=data.time,
                linear=self.linear,
                jitty=self.jity,
            )
            self.post = utils.initialize_post(
                data, params=self.params, priors=self.priors, linear=self.linear
            )
    ...
    def add_planet(self):
        ...
        default_params = utils.initialize_default_pars(
            self.tels,
            jitty=self.jity,
    ...
    def sub_planet(self):
        ...
        default_pars = utils.initialize_default_pars(
            instnames=self.tels, jitty=self.jity
        )
        new_params = radvel.Parameters(new_num_planets, basis=fitting_basis)
    ...
    def fit_orbit(self):
        ...
        if self.polish:
            ...
            if self.manual_grid is not None:
                ...
                subgrid = np.linspace(
                    0.99 * self.manual_grid[peak], 1.01 * self.manual_grid[peak], 9
                )
            elif peak == len(self.periodograms[self.num_planets - 1]) - 1:
                subgrid = np.linspace(
                    self.pers[peak - 1], 2 * self.pers[peak] - self.pers[peak - 1], 9
                )
    ...
    def running_per(self):
        ...
        if self.post.params["dvdt"].vary == True:
            y -= self.post.params["dvdt"].value * (
                x - self.post.likelihood.model.time_base
            )

        if self.post.params["curv"].vary == True:
            y -= (
                self.post.params["curv"].value
                * (x - self.post.likelihood.model.time_base) ** 2
            )
        ...
        for n in np.arange(1, self.num_planets + 1):
            ...
            for i in np.arange(12, nobs + 1):
                freqs = 1.0 / subpers
                power = LombScargle(
                    x[:i], yres[:i], yerr[:i], normalization="psd"
                ).power(freqs)
    ...
    def run_search(
        self, fixed_threshold=None, outdir=None, mkoutdir=True, running=True
    ):
        ...
        while run:
            ...
            print("Before periodogram")
            perioder = periodogram.Periodogram(
                self.post,
            ...
            if perioder.best_bic > perioder.bic_thresh:
                print(
                    f"Planet {self.num_planets + 1} detected. Improved BIC by {perioder.best_bic - perioder.bic_thresh}"
                )
                self.num_planets += 1
                for k in self.post.params.keys():
                    vind = self.post.vector.indices[k]
                ...
                print(
                    f"No planet detected. BIC improvement: {perioder.best_bic - perioder.bic_thresh}"
                )
                self.sub_planet()
        ...
        if self.mcmc == True and (
            self.num_planets != 0 or self.post.params["dvdt"].vary == True
        ):
            self.post.uparams = {}
            self.post.medparams = {}
            self.post.maxparams = {}
            ...
            logpost = utils.initialize_post(
                self.data, params=logparams, priors=self.post.priors
            )
            ...
            if self.mstar is not None:
                self.post = utils.derive(
                    self.post, synthchains, self.mstar, self.mstar_err
                )
            ...
            if self.save_outputs:
                ...
                if self.post.params["curv"].vary == True:
                    labels.append("curv")
                texlabels = [self.post.params.tex_labels().get(l, l) for l in labels]
        ...
        if self.save_outputs:
            ...
            periodograms_plus_pers = np.append(
                [self.pers], list(self.periodograms.values()), axis=0
            ).T
            np.savetxt(
                outdir + "/pers_periodograms.csv",
            ...
            threshs_bics_faps = np.append(
                ...
                [list(self.best_bics.values()), list(self.eFAPs.values())],
                axis=0,
            ).T

            np.savetxt(
                outdir + "/thresholds_bics_faps.csv",
                threshs_bics_faps,
